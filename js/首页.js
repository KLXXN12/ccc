...
var left1 = document.querySelector(".left1");//获取公司概况左边div
...
window.onscroll = function () {
    ...
    if (top > (hh.offsetTop - hh.offsetHeight + 600)) {//书画作品下的第一张图片出场
        hh.style.animation = "zhankai 1s linear 1";
    }
    if (top > (hh2.offsetTop - hh2.offsetHeight + 600)) {//书画作品下的第二张图片出场
        hh2.style.animation = "zhankai 1s linear 1";
    }
    if (top > (hh3.offsetTop - hh3.offsetHeight + 600)) {//书画作品下的第三张出场
        hh3.style.animation = "zhankai 1s linear 1";
    }
    if (top > (hh4.offsetTop - hh4.offsetHeight + 600)) {//书画作品下的第四张图片出场
    ...
    if (top > (masking_out.offsetTop - masking_out.offsetHeight + 1100)) {//公司概括左侧轮播图片的墨水蒙版出场
        masking_out.style.animation = "left1 1s linear 1";
    }
    if (top > (left1.offsetTop - left1.offsetHeight + 1100)) {//公司概括左侧轮播图片出场与执行
    ...
    if (top > (li1.offsetTop - li1.offsetHeight + 2200)) {//收购流程第一个li出场
        li1.style.animation = "right5 1s linear 1";
    }
    if (top > (li2.offsetTop - li2.offsetHeight + 2200)) {//收购流程第二个li出场
        li2.style.animation = "right5 1.2s linear 1";
    }
    if (top > (li3.offsetTop - li3.offsetHeight + 2200)) {//收购流程第三个li出场
        li3.style.animation = "right5 1.4s linear 1";
    }
    if (top > (li4.offsetTop - li4.offsetHeight + 2200)) {//收购流程第四个li出场
        li4.style.animation = "right5 1.6s linear 1";
    }
    if (top > (li5.offsetTop - li5.offsetHeight + 2200)) {//收购流程第五个li出场
